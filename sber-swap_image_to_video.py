...
def sber_swap(src_image_path: Path, target_video_path: Path, output_dir: Path, aligned=False):
    ...
    G = AEI_Net(backbone='unet', num_blocks=3, c_id=512)
    G.eval()
    G.load_state_dict(torch.load('weights/G_unet_3blocks.pth', map_location=torch.device('cpu')))
    G = G.cuda()
    G = G.half()
    ...
    vid_name = target_video_path.name.split('.')[0]
...
def main(pics, vids, output_folder):
    ...
    for src_path in src_li:
        ...
        for target_path in target_li:
            ...
            save_path = f'{output_folder}/{t_name}+{s_name}'
            os.makedirs(save_path, exist_ok=True)
            sber_swap(src_path, target_path, save_path)
