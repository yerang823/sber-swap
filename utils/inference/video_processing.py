...
def crop_frames_and_get_transforms(full_frames: List[np.ndarray],
                ...
                set_target: bool,
                similarity_th: float,
                vid_name: str ) -> Tuple[List[Any], List[Any]]:
    """
    ...
    try:
        kps_li = np.load(f'../result/tmp_npy/{vid_name}_kps.npy', allow_pickle=True)
        np_exist=True
        print('\nFOUND EXISTING NPY!\n')
...
def get_final_video_encode(final_frames: List[np.ndarray],
                    crop_frames: List[np.ndarray],
                    full_frames: List[np.ndarray],
                    tfm_array: List[np.ndarray],
                    output_frames: str,
    ...
    for i in tqdm(range(len(full_frames))):
        ...
        for j in range(len(crop_frames)):
            try:
                ...
                pass
        cv2.imwrite(f"{output_frames}/frame_{i}.jpg", result_frames[i])        
        out.write(result_frames[i])

    out.release()
    os.system(f"ffmpeg -i {OUT_VIDEO_NAME} -vcodec libx264 {OUT_VIDEO_NAME[:-4]}_encode.mp4 -y")
    return f'{OUT_VIDEO_NAME[:-4]}_encode.mp4'

def save_final_frames(final_frames: List[np.ndarray],
                    crop_frames: List[np.ndarray],
                    full_frames: List[np.ndarray],
                    tfm_array: List[np.ndarray],
                    output_frames: str,
    ...
    for i in tqdm(range(len(full_frames))):
        ...
        for j in range(len(crop_frames)):
            try:
                ...
                pass
        frame_idx = f'{i}'.zfill(7)
        cv2.imwrite(f"{output_frames}/{frame_idx}.jpg", result_frames[i])
