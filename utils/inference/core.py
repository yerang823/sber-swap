...
def model_inference(full_frames: List[np.ndarray],
                    ...
                    BS=60,
                    half=True,
                   vid_name=None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    crop_frames_list, tfm_array_list = crop_frames_and_get_transforms(full_frames, target_embeds, app, netArc, crop_size, set_target, similarity_th=similarity_th, vid_name=vid_name)
